...
void postOrderIterativeS1(BSTNode *root)
{
    ...
    do {
        ...
        while (current == NULL && !isEmpty(&stack)) {
            current = peek(&stack); // Peek the top node from the stack
            if (current->right == NULL || current->right == prev) { // If the right child is NULL or already visited
                printf("%d ", current->item); // Print the node

...
{
	// add your code here
	// Calculate new size for front list
    int newsize = (ll->size + 1) / 2; 
    ...
    // Traverse the list until newsize
    for (int i = 0; i < newsize && cur != NULL; i++) {
        prev = cur;
        cur = cur->next;
        resultFrontList->size++; // Increment size for front list
    }

    // Set the end of the front list
    if (prev != NULL) {
        prev->next = NULL;
    }

    // Assign the rest of the list to the back list
    resultBackList->head = cur;
    resultBackList->size = ll->size - resultFrontList->size;

    // Ensure ll points to NULL to avoid double freeing memory
    ll->head = NULL;
    ll->size = 0;
}
//////////////////////////////////////////////////////////////////////////////////

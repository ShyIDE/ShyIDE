...
void alternateMergeLinkedList(LinkedList *ll1, LinkedList *ll2)
{
    ...
    if (ll1 == NULL || ll2 == NULL) {
        printf("One or both input linked lists are NULL.\n");
        return;
    }
    ...
    // Merge nodes from ll2 into ll1
    while (current1 != NULL && current2 != NULL) {
        ListNode *next1 = current1->next;
        ListNode *next2 = current2->next;

        // Insert node from ll2 into ll1
        insertNode(ll1, count * 2 + 1, current2->item);
        count++;

        // Remove node from ll2
        removeNode(ll2, 0);
    ...
    // Handle remaining nodes in ll2
    while (current2 != NULL) {
        // Insert remaining nodes from ll2 into ll1
        insertNode(ll1, count * 2 + 1, current2->item);
        count++;

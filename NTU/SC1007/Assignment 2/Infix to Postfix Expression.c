...
void infixtoPostfix(char *infix, char *postfix)
{
   // add your code here
    ...
    while (infix[i] != '\0') {
        ...
        if (currentChar == '(') {
            push(&operators, currentChar);
        } else if (currentChar == ')') {
            // Pop operators from the stack and append to postfix expression until '(' is encountered
            while (!isEmptyStack(&operators) && peek(&operators) != '(') {
                postfix[j++] = pop(&operators);
            }
            // Pop '(' from the stack but do not append it to postfix expression
            pop(&operators);
        } else if (currentChar == '+' || currentChar == '-' || currentChar == '*' || currentChar == '/') {
            // Operator encountered
            while (!isEmptyStack(&operators) && precedence(peek(&operators)) >= precedence(currentChar) && peek(&operators) != '(') {
                postfix[j++] = pop(&operators);
            }
            push(&operators, currentChar);
        } else {
    ...
    // Add null terminator to the end of postfix expression
    postfix[j] = '\0';
}
